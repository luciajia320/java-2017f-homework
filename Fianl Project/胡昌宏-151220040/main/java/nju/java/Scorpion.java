...
public class Scorpion extends Creature implements Runnable {
    ...
    public Scorpion(int x, int y, int rank, Field field) {
        super(x, y,field);
        this.isGoodMan = false;
        this.field = field;
        this.rank = rank;
        this.state = State.left;
        URL loc = this.getClass().getClassLoader().getResource("Scorpion-left.png");
    ...
    public void moveRight() {
    	if(x() >= 14*field.getSpace() + field.getOffset()) {
    		state = State.left;
            setImage(new ImageIcon(getClass().getClassLoader().getResource("Scorpion-left.png")).getImage());
            moveLeft();
        }
    	else
    		super.moveRight();
    ...
    public void moveLeft() {
    	if(x() <= 0*field.getSpace() + field.getOffset()) {
    		state = State.right;
            setImage(new ImageIcon(getClass().getClassLoader().getResource("Scorpion-right.png")).getImage());
            moveRight();
        }
    	else
    		super.moveLeft();
