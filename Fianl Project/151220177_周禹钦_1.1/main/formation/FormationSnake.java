...
public class FormationSnake implements Formation{
    private Creature[][] matrix=new Creature[8][8];
    private int[] usableposition={04,14,24,34,44,54,64,74};
    private int kind=6;
    @Override
    ...
    public FormationSnake(boolean monster){
        if(monster){
            for(int i=0;i<8;i++)
                for(int j=0;j<8;j++)
                    matrix[i][j]=new GrassGround();
            matrix[0][3]=new MonsterServent();
            matrix[1][3]=new MonsterServent();
            ...
            matrix[1][4]=new HuLuWa(COLOR.赤, SENIORITY.一);
            ...
            matrix[5][4]=new HuLuWa(COLOR.青, SENIORITY.五);
            matrix[6][4]=new HuLuWa(COLOR.蓝, SENIORITY.六);
            matrix[7][4]=new HuLuWa(COLOR.紫, SENIORITY.七);
        }
    }
}
