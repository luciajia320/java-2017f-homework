...
public class FormationBucket implements Formation{
    private Creature[][] matrix=new Creature[8][8];
    private int[] usableposition={23,24,32,35,42,45,53,54};
    private int kind=2;
    @Override
    ...
    public FormationBucket(boolean monster){
        if(monster){
            for(int i=0;i<8;i++)
                for(int j=0;j<8;j++)
            ...
            matrix[3][2]=new MonsterScorpion();
            matrix[3][5]=new MonsterServent();
            matrix[4][2]=new MonsterSnake();
