...
public class Xiaolouluo extends Thing2D implements Runnable {
    private Field field;
   ...
   // private int sleepTime=5000;
	...
	public Xiaolouluo(int x, int y, Field field,int rank) {
        super(x, y);
	...
	public void move(int x, int y) {
		...
		int prePositionY=(this.y()-30)/20;
         ...
         if(field.getPositions()[positionX][positionY].getHolder()!=null) {
         	...
         	if(toObj.equals("Xiezijing")||toObj.equals("Xiaolouluo")||toObj.equals("Shejing"))//friend
         		return;
         	else
         		//enemy , attack
         	{
        			...
        			try {
						...
						//field.setCreature(this, positionX, positionY);
						 synchronized(this) {
							 ...
							 if(!field.getGround().getTextArea().getText().contains("小喽啰"+rank+"号"))
							 field.getGround().getTextArea().append("小喽啰"+rank+"号阵亡\n");
							...
							field.setCreature(this, positionX, positionY);
							 ...
							 synchronized(this) {
								field.getBuffer().append("battle: louluo"+rank+" win hulu"+huluRank+"\r\n");
            			...
            			try {
							...
							//field.setCreature(this, positionX, positionY);
							 synchronized(this) {
								field.getBuffer().append("battle: louluo"+rank+" lose grandpa\r\n");
								 if(!field.getGround().getTextArea().getText().contains("小喽啰"+rank+"号"))
								 field.getGround().getTextArea().append("小喽啰"+rank+"号阵亡\n");
								 field.getDeadSquare().put(this.x(), this.y());
								 field.getPositions()[prePositionX][prePositionY].setHolder(null);
								...
								field.setCreature(this, positionX, positionY);
								 ...
								 synchronized(this) {
									field.getBuffer().append("battle: louluo"+rank+" win grandpa\r\n");
         		...
         		//enemy , attack
         	{
         		if(toObj.equals("HuLuWa")) {
         	   		for(String s:field.getBattleStrs()) {
	         			String[] arr=s.split(" ");
	         			if(arr[1].contains("louluo"+rank)&&arr[3].contains("hulu")) {//交火
	         				int ranktmp=arr[3].charAt(4)-'0';
	         				
	         				if(arr[2].contains("win")) {//击败葫芦娃
         		   		...
         		   		for(String s:field.getBattleStrs()) {
		         			String[] arr=s.split(" ");
		         			if(arr[1].contains("louluo"+rank)&&arr[3].contains("grandpa")) {//交火
