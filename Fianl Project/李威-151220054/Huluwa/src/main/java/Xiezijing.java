...
public class Xiezijing extends Thing2D implements Runnable {
    private Field field;
		...
		if(field.isFromFile()==false) {
			...
			int prePositionY=(this.y()-30)/20;
	         ...
	         if(field.getPositions()[positionX][positionY].getHolder()!=null) {
	         	...
	         	if(toObj.equals("Shejing")||toObj.equals("Xiaolouluo"))//friend
	         		return;
	         	else
	         		//enemy , attack
	         	{
	         		if(toObj.equals("HuLuWa")) {
	         			...
	         			try {
							...
							alive=false;
							 ...
							 synchronized(this) {
								 field.getBuffer().append("battle: xiezijing lose hulu"+huluRank+"\r\n");
								...
								field.setCreature(this, positionX, positionY);
								 ...
								 synchronized(this) {
									 field.getBuffer().append("battle: xiezijing win hulu"+huluRank+"\r\n");
	        			...
	        			try {
							...
							alive=false;
							 ...
							 synchronized(this) {
								 field.getBuffer().append("battle: xiezijing lose grandpa\r\n");
								if(!field.getGround().getTextArea().getText().contains("蝎子精"))
								 field.getGround().getTextArea().append("蝎子精阵亡\n");
								 field.getDeadSquare().put(this.x(), this.y());
								 field.getPositions()[prePositionX][prePositionY].setHolder(null);
								...
								field.setCreature(this, positionX, positionY);
								 ...
								 synchronized(this) {
									 field.getBuffer().append("battle: xiezijing win grandpa\r\n");
	         		...
	         		//enemy , attack
	         	{
	         		if(toObj.equals("HuLuWa")) {
	         			for(String s:field.getBattleStrs()) {
		         			String[] arr=s.split(" ");
		         			if(arr[1].contains("xiezijing")&&arr[3].contains("hulu")) {//交火
	        			...
	        			for(String s:field.getBattleStrs()) {
		         			String[] arr=s.split(" ");
		         			if(arr[1].contains("xiezijing")&&arr[3].contains("grandpa")) {//交火
		         				
		         				
		         				if(arr[2].contains("win")) {//击败爷爷
		         					field.getGrandpa().setAlive(false);
				         			field.setCreature(this, positionX, positionY);
				         			 synchronized(this) {
