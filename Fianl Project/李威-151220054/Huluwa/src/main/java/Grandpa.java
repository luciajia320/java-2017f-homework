...
public class Grandpa extends Thing2D implements Runnable {
    ...
    private void showStatus() {
		System.out.println("Grandpa运行时候的holder分布");
		int total=0;
        for(int i=0;i<16;i++) {
        	for(int j=0;j<16;j++) {
        		if(field.getPositions()[j][i].getHolder()!=null)
        			{
        			System.out.print("1 ");
        			total++;
        			}
        		   // System.out.print(positions[j][i].getHolder().getClass().getSimpleName());
        		else
             		...
             		if(toObj.equals("Shejing")) {
             			...
             			try {
							...
							if(a==0) {
								...
								field.setCreature(this, positionX, positionY);
								 synchronized(this) {
									 field.getBuffer().append("battle: grandpa win shejing\r\n");
								...
								alive=false;
								synchronized(this) {
									field.getBuffer().append("battle: grandpa lose shejing\r\n");
             			...
             			try {
							...
							if(a==0) {
								...
								field.setCreature(this, positionX, positionY);
								 synchronized(this) {
									field.getBuffer().append("battle: grandpa win xiezijing\r\n");
								...
								alive=false;
								synchronized(this) {
									field.getBuffer().append("battle: grandpa lose xiezijing\r\n");
									 if(!field.getGround().getTextArea().getText().contains("爷爷"))
									 field.getGround().getTextArea().append("爷爷阵亡\n");
									 field.getDeadSquare().put(this.x(), this.y());
							    	 field.getPositions()[prePositionX][prePositionY].setHolder(null);
             			...
             			try {
							...
							if(a==0) {
								alive=false;
								synchronized(this) {
									field.getBuffer().append("battle: grandpa lose louluo"+louluoRank+"\r\n");
									 if(!field.getGround().getTextArea().getText().contains("爷爷"))
									 field.getGround().getTextArea().append("爷爷阵亡\n");
									 field.getDeadSquare().put(this.x(), this.y());
							    	 field.getPositions()[prePositionX][prePositionY].setHolder(null);
							...
							field.setCreature(this, positionX, positionY);
							 synchronized(this) {
								field.getBuffer().append("battle: grandpa win louluo"+louluoRank+"\r\n");
              		...
              		//enemy , attack
              	{
              		if(toObj.equals("Shejing")) {
              			...
              			for(String s:field.getBattleStrs()) {
		         			String[] arr=s.split(" ");
		         			if(arr[1].contains("grandpa")&&arr[3].contains("shejing")) {//交火
		         				if(arr[2].contains("win")) {//击败蛇精
		         					field.getShejing().setAlive(false);
				         			field.setCreature(this, positionX, positionY);
				         			 synchronized(this) {
              			...
              			for(String s:field.getBattleStrs()) {
		         			String[] arr=s.split(" ");
		         			if(arr[1].contains("grandpa")&&arr[3].contains("xiezijing")) {//交火
		         				if(arr[2].contains("win")) {//击败
		         					field.getXiezijing().setAlive(false);
				         			field.setCreature(this, positionX, positionY);
				         			 synchronized(this) {
				         				// field.getBuffer().append("battle: hulu"+rank+" win shejing\r\n");
              			...
              			for(String s:field.getBattleStrs()) {
		         			String[] arr=s.split(" ");
		         			if(arr[1].contains("grandpa")&&arr[3].contains("louluo")) {//交火
