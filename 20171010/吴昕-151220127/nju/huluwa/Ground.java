...
public class Ground {
	...
	public void clear() throws ChangeFormationException {
		if (exists == null) throw new ChangeFormationException("Clear error");
		exists.removeAll(exists);
	}
	...
	private boolean conflict(Formation f1, Formation f2) {
		if (contains(f1, f2)) return true;
		if (contains(f2, f1)) return true;
		if (overlap(f1, f2)) return true;
		if (overlap(f2, f1)) return true;
