...
public class Space {
    ...
    public void remove(Formation formation){
        exists.remove(formation);
    }

    public void layout(Formation formation, Position location) throws TooCrowdedException {
        ...
        Position loc = formation.getLocation();
        ...
        for (Object o : exists) {
            ...
            for (int i = 0; i < f.getWidth(); i++) {
                for (int j = 0; j < f.getHeight(); j++) {
                    if(f.getContent()[i][j] != null) {
                        content[i + f.getLocation().getX()][j + f.getLocation().getY()]
                            = f.getContent()[i][j].toString();
                    }
                }
            }
        }
